...
#include "debug.hpp"
Motors::Motors(Sonic *left, Sonic *right, int M1F, int M1B, int M2F, int M2B,
               int M1S, int M2S)
    : pin_m1f(M1F),
      pin_m1b(M1B),
      pin_m2f(M2F),
...
// Force the robot to stop whatever it is doing with its motors and step away
// from that puck!
...
Motors::MotorStateMachine::MotorStateMachine()
    : time(0), current(None), cont(None) {}

void Motors::MotorStateMachine::continuous_schedule(States s) { cont = s; }

void Motors::MotorStateMachine::reset_timer() { time = millis(); }
    ...
    switch (MSM.current) {
        ...
        case States::GoAroundDecide: {
            ...
            if ((DistanceR >= SONIC_STUCK_THRESHHOLD ||
                 DistanceL >= SONIC_STUCK_THRESHHOLD)) {
                if (!is_stuck) {
                    is_stuck = true;
                    stuck_time = millis();
                }
            ...
            if ((DistanceR < SONIC_WALL_THRESHHOLD ||
                 DistanceL < SONIC_WALL_THRESHHOLD) ||
                (is_stuck && (millis() - stuck_time >= 3000))) {
...
const Motors::MotorStateMachine::States
    Motors::MotorStateMachine::next[States::LENGTH] = {
        States::None,  // None = 0,
        States::None,  // GoAroundDecide, -> (GoAroundCruise | GoAroundStepBack
                       // | None)
        States::None,  // GoAroundCruise,
        States::GoAroundTurnRight,  // GoAroundStepBack,
        States::None,               // GoAroundTurnRight,
        States::None,  // FollowTheWallsDecide, -> (FollowTheWallsCruise |
